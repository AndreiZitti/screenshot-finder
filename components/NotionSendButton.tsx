...
export default function NotionSendButton({
  type,
  name,
  description,
  transcription,
  ...
  return (
    <div className="relative" ref={pickerRef}>
      <button
        ...
        className={`flex min-h-11 min-w-11 shrink-0 items-center justify-center rounded p-2 transition-colors disabled:opacity-50 ${
          status === 'success'
            ? 'text-green-500'
            : status === 'error'
        ...
        aria-label={isConfigured ? 'Send to Notion' : 'No Notion connection configured'}
        ...
        <div className="absolute right-0 top-full mt-1 z-50 w-48 rounded-lg border border-gray-200 bg-white py-1 shadow-lg">
          ...
          {connections.map((connection) => (
            <button
              ...
              className="flex min-h-11 w-full items-center gap-2 px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-50"
            >
